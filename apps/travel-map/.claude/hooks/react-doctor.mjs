...
/**
 * Reads a file or standard input without allowing hook diagnostics to crash.
 * @param {string | number} source - The file path or descriptor to read
 * @returns {string} The file contents, or an empty string when reading fails
 */
...
/**
 * Checks whether the completed tool call may have changed source files.
 * @param {Record<string, unknown>} input - The hook payload
 * @returns {boolean} Whether React Doctor should scan the changes
 */
const shouldScan = (input) => {
  ...
  if (eventName === "PostToolBatch") {
    ...
    return toolCalls.some((toolCall) =>
      EDIT_TOOL_NAMES.has(toolCall.tool_name),
    );
  }
...
/**
 * Runs React Doctor with the first available local or package runner.
 * @param {string} outputPath - The temporary diagnostic output path
 * @returns {number | null} The process exit status
 */
const runReactDoctor = (outputPath) => {
  ...
  for (const command of commands) {
    ...
    try {
      ...
      /* Best-effort diagnostic output; hook status remains authoritative. */
    }
...
/**
 * Removes temporary files without failing an agent hook.
 * @param {...string} paths - The temporary paths to remove
 * @returns {void}
 */
const cleanup = (...paths) => {
  for (const path of paths) {
    try {
      ...
      /* Temporary files may already be absent after an interrupted hook run. */
    }
  }
};

/**
 * Reads the hook event, runs React Doctor when needed, and reports regressions.
 * @returns {void}
 */
const main = () => {
  let input;
  ...
  /*
   * The write above is best-effort (unwritable tmpdir), so the read is too — a
   * hook must never crash the agent loop with a stack trace.
   */
